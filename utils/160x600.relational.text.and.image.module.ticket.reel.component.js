{
            type: 'relational.text.and.image.module.ticket.reel.component',

            frameDuration: 5000,

            transition: { cut: false, fade: true, topToBottom: false, rightToLeft: false, bottomToTop: false, leftToRight: false },
            transitionTime: { time: 750 },

            background: { colour: '#003464', animationTime: 500, delay: 750 },

            headlineColour: '#ffffff',
            headlinePosition: { x: 80, y: 85 },
            headlineText: 'Frame_2_Headline_standard_size',
            headlineAnimation: { fade: true, slideLeftToRight: false, slideRightToLeft: false, slideTopToBottom: false, zoom: false, fadeWithBounce: false, pulse: false, none: false },
            headlineAnimationTime: { time: 500, delay: 750 },

            relationalColour: '#ffffff',
            relationalPosition: { x: 80, y: 447 },
            relationalText: 'Frame_2_Relational_standard_size',
            relationalAnimation: { fade: true, slideLeftToRight: false, slideRightToLeft: false, slideTopToBottom: false, zoom: false, fadeWithBounce: false, pulse: false, none: false },
            relationalAnimationTime: { time: 500, delay: 750 },

            imageMask: '02_relational_text_and_image_module_reel_ticket_mask.png',
            imageOnTheLeft: '02_relational_text_and_image_module_reel_ticket_left.png',
            imageInTheCenter: '02_relational_text_and_image_module_reel_ticket_center.png',
            imageOnTheRight: '02_relational_text_and_image_module_reel_ticket_right.png',

            txOnTheLeft: '<span style="line-height: 11px">&nbsp;</span><br>View detail<br>Programme name',
            txInTheCenter: '<span style="line-height: 11px">&nbsp;</span><br>View detail<br>Programme name',
            txOnTheRight: '<span style="line-height: 11px">&nbsp;</span><br>View detail<br>Programme name',

            logoOnTheLeft: 'logo.png',
            logoInTheCenter: 'logo.png',
            logoOnTheRight: 'logo.png',

            imageOnTheLeftReverse: '1x1.png',
            imageInTheCenterReverse: '1x1.png',
            imageOnTheRightReverse: '1x1.png',

            txOnTheLeftReverse: '&nbsp;',
            txInTheCenterReverse: '&nbsp;',
            txOnTheRightReverse: '&nbsp;',

            logoOnTheLeftReverse: '1x1.png',
            logoInTheCenterReverse: '1x1.png',
            logoOnTheRightReverse: '1x1.png',

            reelAnimation: { last: true, all: false, middle: false, none: false },
            reelAnimationTime: { time: 500, delay: 750, delayBeforeFlip: 500 },
            reelPosition: { left: false, right: true },
            reelFlipAnimation: { left: false, middle: false, right: false },

            showCallToAction: { visible: true, delay: 750 }
        },